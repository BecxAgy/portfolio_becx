  ...
  h-max bottom-0  mt-auto xl:right-[2%] z-50 top-0 w-full
   xl:w-16 xl:max-w-md xl:h-screen'>
    ...
    justify-between xl:justify-center gap-y-10 px-4 md:px-40
     xl:px-8 h-[80px] xl:h-max py-8 bg-white/10
      backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'>
      {navData.map((link, index)=> (<Link className={`${link.path ===pathName && 'text-accent '} relative flex items-center 
       hover:text-accent transition-all group duration-300`} key={index} href={link.path} >
       
       <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
          <div className='bg-white relative flex text-primary 
          items-center p-[6px] rounded-[5px]'>
